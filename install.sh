...
VMESS_WSPATH=${VMESS_WSPATH:-'/am'}
VLESS_WSPATH=${VLESS_WSPATH:-'/bl'}
URL=${USER}.alwaysdata.net

wget -q -O $TMP_DIRECTORY/config.json https://raw.githubusercontent.com/babama1001980/alway/main/config.json
wget -q -O $TMP_DIRECTORY/sing-box-1.5.3-windows-amd64.zip https://github.com/SagerNet/sing-box/releases/download/v1.5.3/sing-box-1.5.3-windows-amd64.zip
unzip -oq -d $HOME $TMP_DIRECTORY/sing-box-1.5.3-windows-amd64.zip geoip.dat geosite.dat geoip-only-cn-private.dat

sed -i "s#UUID#$UUID#g;s#VMESS_WSPATH#$VMESS_WSPATH#g;s#VLESS_WSPATH#$VLESS_WSPATH#g;s#30000#8414#g;s#40000#8424#g;s#127.0.0.1#0.0.0.0#g" $TMP_DIRECTORY/config.json
...
ProxyPass "${VMESS_WSPATH}" "ws://services-${USER}.alwaysdata.net:8414${VMESS_WSPATH}"
ProxyPassReverse "${VMESS_WSPATH}" "ws://services-${USER}.alwaysdata.net:8414${VMESS_WSPATH}"
ProxyPass "${VLESS_WSPATH}" "ws://services-${USER}.alwaysdata.net:8424${VLESS_WSPATH}"
ProxyPassReverse "${VLESS_WSPATH}" "ws://services-${USER}.alwaysdata.net:8424${VLESS_WSPATH}"
EOF
)
